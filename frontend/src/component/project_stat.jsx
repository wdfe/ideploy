...
class Stat extends React.Component {
  ...
  fetch(startTime,endTime) {
    ...
    //用户部署统计
    let project_id=this.props.location.query.id;
    this.setState({
      projectName: this.props.location.query.name
    })
    this.dataAjax('/home/history/get_user_stat?id='+project_id+'&startTime='+startTime+'&endTime='+endTime, '部署系统用户部署统计', 'userOption');
    //环境统计
    this.dataAjax('/home/history/get_env_stat?id='+project_id+'&startTime='+startTime+'&endTime='+endTime, '部署系统环境部署统计', 'envOption');
  }
  componentDidMount() {
    this.fetch();
  ...
  render() {
    ...
    return (
      ...
      <Row>
       <div className="section-title" style={{textAlign:'center'}}>{this.state.projectName}项目的部署统计：</div>
      </Row>
        <div>
