...
class UserStat extends React.Component {
  ...
  fetch(startTime,endTime) {
    ...
    //用户部署统计
    let user=this.props.location.query.id;
    this.setState({
      projectName: user
    })
    //总统计
    this.dataAjax('/home/history/get_total_stat?startTime='+startTime+'&endTime='+endTime+'&user='+user, '部署系统项目部署统计', 'totalOption');
    //环境统计
    this.dataAjax('/home/history/get_env_stat?startTime='+startTime+'&endTime='+endTime+'&user='+user, '部署系统环境部署统计', 'envOption');
  }
  componentDidMount() {
    this.fetch();
  ...
  render() {
    ...
    return (
      ...
      <Row>
       <div className="section-title" style={{textAlign:'center'}}>{this.state.projectName}的部署统计：</div>
      </Row>
        <div>
...
export default Form.create({})(UserStat);
